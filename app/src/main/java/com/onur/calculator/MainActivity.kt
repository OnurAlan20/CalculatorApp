...
class MainActivity : AppCompatActivity() {
    ...
    val aryDigit = arrayOf("1","2","3","4","5","6","7","8","9","0")
    val dot ="."
    ...
    fun onDelete(view: View){
        tvInput?.setText(tvInput?.text?.dropLast(1))

        if (tvInput?.text?.takeLast(1) in aryDigit ) {
            lastNumeric = true
            lastDot = false
        }
        else if (tvInput?.text?.takeLast(1) == dot){
            lastNumeric = false
            lastDot = true
        }
        else{
